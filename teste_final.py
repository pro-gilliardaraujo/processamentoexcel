...
if df_tt is not None and df_pc is not None:
    ...
    if colunas_apenas_tt:
        print(f"\nColunas apenas em Codigo_Base_TT.py: {sorted(list(colunas_apenas_tt))}")
    
    if colunas_apenas_pc:
        print(f"\nColunas apenas em processamento_completo_transbordos.py: {sorted(list(colunas_apenas_pc))}")
